  ...
  try {
    ...
    if (testimonials.length === 0) {
      swiperWrapper.innerHTML = `
        <div class="swiper-slide">
          <div class="testimonial-card text-center p-4">
            <p class="text-gray-600">No hay testimonios aún. ¡Sé el primero en dejar tu opinión!</p>
          </div>
        </div>
      `;
    } else {
      testimonials.forEach(testimonial => {
        ...
        slide.innerHTML = `
          <div class="testimonial-card bg-white p-6 rounded-lg shadow-md h-full flex flex-col">
            <div class="flex flex-col items-center flex-grow">
              <img src="${formatImageUrl(testimonial.avatar)}" 
                   alt="${testimonial.name}" 
                   class="w-20 h-20 rounded-full object-cover mb-4"
                   onerror="this.src='https://via.placeholder.com/150'">
              <h3 class="text-xl font-semibold text-gray-800 text-center">${testimonial.name}</h3>
              <p class="text-sm text-gray-500 mb-2 text-center">${testimonial.role}</p>
              <p class="text-gray-600 italic text-center flex-grow">"${testimonial.comment}"</p>
            </div>
          </div>
        `;
...
if (testimonialForm) {
  testimonialForm.addEventListener("submit", async (e) => {
    ...
    try {
      ...
      const response = await fetch("https://aly-mbelleza-backend.onrender.com/api/testimonials", {
        method: "POST",
        body: formData,
...
document.getElementById("contactForm").addEventListener("submit", async (e) => {
  ...
  try {
    ...
    // URL actualizada para desarrollo/producción
    const apiUrl = window.location.hostname === 'localhost' 
      ? 'http://localhost:5000/api/send-email'
      : 'https://tudominio.com/api/send-email';
    ...
    showNotification(
      "success",
      "¡Mensaje enviado!",
      result.message || "Gracias por contactarnos. Te responderemos pronto."
    );
...
function showNotification(type, title, message) {
  ...
  } border-t-2 rounded-lg p-4 dark:${
    type === "success" ? "bg-teal-800/30" : "bg-red-800/30"
  }`;
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  // Aplicar animación de entrada con Tailwind
  setTimeout(() => {
  ...
  document.querySelectorAll(".close-btn").forEach((button) => {
    button.addEventListener("click", () => {
      ...
      setTimeout(() => {
        banner.style.display = "none"; // Oculta después de la animación
      }, 500);
    });
  });
});
