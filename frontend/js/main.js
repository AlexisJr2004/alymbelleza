...
// =============== Manejo de Notificaciones de Citas Usuario ================
document.addEventListener("DOMContentLoaded", function () {
  // Selección de elementos
  const notifDiv = document.getElementById("appointment-notification");
  const bellBtn = document.getElementById("notification-bell");
  const dropdown = document.getElementById("appointment-dropdown");
  const appointmentsList = document.getElementById("appointments-list");
  const emptyMsg = document.getElementById("appointments-empty");
  const notifyDot = document.getElementById("notify-dot");
  // Revisar que existan para no romper otras páginas
  if (!notifDiv || !bellBtn || !dropdown) return;

  // === Mostrar el icono solo si hay usuario logueado y tiene token ===
  const user = JSON.parse(localStorage.getItem("user"));
  ...
  // --- Abrir/cerrar dropdown con click fuera para cerrar ---
  let open = false;
  bellBtn.addEventListener("click", function (e) {
    ...
    if (open) {
      fetchAndRenderAppointments(); // Cargar citas cuando abre
    }
  });
  // Cerrar el dropdown si haces click fuera
  document.addEventListener("click", function (e) {
    if (!notifDiv.contains(e.target)) {
      dropdown.classList.add("hidden");
      open = false;
    }
  });

  // --- Traer y mostrar citas del usuario ---
  async function fetchAndRenderAppointments() {
    appointmentsList.innerHTML = '';
    emptyMsg.classList.add("hidden");

    try {
      const res = await fetch("https://aly-mbelleza-backend.onrender.com/api/appointments/me", {
        headers: {
      ...
      if (!res.ok) throw new Error("Error obteniendo tus citas.");
      const json = await res.json();
      const appointments = json.data || [];
      
      /* Mostrar/ocultar puntito de notificación si hay cita futura (hoy o adelante) */
      const now = new Date();
      let hasFuture = false;
      appointments.forEach(app => {
        const dt = new Date(app.date + "T" + (app.time || "00:00"));
        if (dt >= now) hasFuture = true;
      });
      notifyDot && notifyDot.classList.toggle("hidden", !hasFuture);
      ...
      appointments
        ...
        .forEach(app => {
          if (shown >= 10) return; // Limitar a 10 para no saturar
          shown++;
          const dt = new Date(app.date + "T" + (app.time || "00:00"));
          const expired = dt < now;
          const citaDiv = document.createElement("div");
          citaDiv.className = `p-2 rounded-lg ${expired
            ? "bg-gray-100 text-gray-400"
            : "bg-purple-50 border-l-4 border-purple-400 text-purple-900 shadow-sm"} flex flex-col mb-2`;
          citaDiv.innerHTML = `
            <div class="flex items-center justify-between pb-1">
              <span class="font-medium ${expired ? "line-through" : ""}">
                ${app.service}
              </span>
              <span class="text-xs ml-2 ${expired ? "text-gray-400" : "text-purple-600"}">
            ...
            ${app.notes
              ? `<div class="mt-1 text-xs italic text-gray-500">"${app.notes}"</div>`
              : ""}
          `;
          appointmentsList.appendChild(citaDiv);
        });
      // Si no hay ninguna cita futura
      if (!hasFuture && appointments.length) {
        setTimeout(() => notifyDot && notifyDot.classList.add("hidden"), 100);
      }
    } catch (err) {
      appointmentsList.innerHTML = '';
      emptyMsg.classList.remove("hidden");
      emptyMsg.textContent = err.message;
      notifyDot && notifyDot.classList.add("hidden");
      // Si expiró sesión, limpiar usuario y redirigir o mostrar mensaje
